...
const PaymentModalSkeleton = () => (
  <div className="grid gap-4 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]">
    <div className="space-y-3">
      ...
      {[0, 1, 2].map((item) => (
        <div key={item} className="h-24 animate-pulse rounded-3xl border border-gray-200 bg-gray-100" />
      ))}
    </div>
    <div className="h-96 animate-pulse rounded-3xl border border-gray-200 bg-gray-100" />
  </div>
);
    ...
    <button
      ...
      className={`w-full rounded-3xl border p-4 text-left transition ${
        active
          ...
          : 'border-gray-200 bg-white text-gray-950 hover:border-gray-300 hover:shadow-sm'
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className={`mt-1 text-xs ${active ? 'text-white/62' : 'text-gray-500'}`}>
            Vente #{String(sale._id || '').slice(-6)} · {formatShortDate(sale.saleDate || sale.createdAt)}
          </p>
        </div>
        <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${active ? 'bg-white/12 text-white' : 'bg-gray-100 text-gray-700'}`}>
          Solde
        </span>
      </div>
      <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
        <div className={`rounded-2xl px-3 py-2 ${active ? 'bg-white/10' : 'bg-gray-50'}`}>
          <span className={active ? 'text-white/60' : 'text-gray-500'}>Restant</span>
          <p className="mt-1 text-sm font-semibold">{formatAmount(balance)}</p>
        </div>
        <div className={`rounded-2xl px-3 py-2 ${active ? 'bg-white/10' : 'bg-gray-50'}`}>
          <span className={active ? 'text-white/60' : 'text-gray-500'}>Payé</span>
      ...
      footer={
        <>
          ...
          <button
            ...
            className="min-h-[44px] w-full rounded-2xl bg-gray-950 px-4 py-3 font-semibold text-white transition hover:bg-gray-800 disabled:cursor-not-allowed disabled:bg-gray-300 disabled:text-gray-600 flex items-center justify-center gap-2 sm:w-auto"
          >
            {isSubmitting ? (
              <>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]">
          <aside className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <p className="mt-1 text-xl font-semibold text-gray-950">{paymentStats.count}</p>
              </div>
              <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 text-xs font-medium text-gray-500">
                  <Banknote size={14} />
                ...
                <p className="mt-1 text-lg font-semibold text-gray-950">{formatAmount(paymentStats.outstanding)}</p>
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-gray-200 bg-white p-3 shadow-sm">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="min-h-[44px] w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 pl-10 text-sm text-gray-950 outline-none transition focus:border-gray-400 focus:bg-white focus:ring-4 focus:ring-gray-900/5"
                />
              </div>
            ...
            <div className="max-h-[42dvh] space-y-3 overflow-y-auto pr-1 lg:max-h-[min(62dvh,620px)]">
              {payableSales.length === 0 ? (
                <div className="rounded-3xl border border-dashed border-gray-300 bg-white p-6 text-center">
                  ...
                  <p className="mt-3 text-sm font-semibold text-gray-900">Aucune vente à encaisser</p>
                  <p className="mt-1 text-sm text-gray-500">Toutes les ventes sont soldées.</p>
                </div>
              ) : filteredSales.length === 0 ? (
                <div className="rounded-3xl border border-dashed border-gray-300 bg-white p-6 text-center">
                  <Search className="mx-auto h-8 w-8 text-gray-400" />
                  <p className="mt-3 text-sm font-semibold text-gray-900">Aucun résultat</p>
                  <p className="mt-1 text-sm text-gray-500">Essayez un autre nom ou numéro de vente.</p>
                </div>
              ) : (
          ...
          <section className="min-w-0">
            {selectedSaleData ? (
              ...
              <div className="flex min-h-[320px] flex-col items-center justify-center rounded-3xl border border-dashed border-gray-300 bg-gray-50 px-6 text-center">
                <Wallet className="h-10 w-10 text-gray-400" />
                <h3 className="mt-4 text-base font-semibold text-gray-950">Sélectionnez une vente</h3>
