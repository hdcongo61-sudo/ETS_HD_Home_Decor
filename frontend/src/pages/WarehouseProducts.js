                ...
                <div className="flex flex-col gap-4 mb-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
                    <div className="bg-gray-50 rounded-xl p-3 sm:p-4 border border-gray-100 min-h-[44px] flex flex-col justify-center">
                      ...
                      <p className="text-base sm:text-lg font-semibold text-gray-800 mt-0.5">{formatNumber(warehouse.totalProducts)}</p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-3 sm:p-4 border border-gray-100 min-h-[44px] flex flex-col justify-center">
                      <p className="text-xs sm:text-sm text-gray-500">Stock</p>
                      <p className="text-base sm:text-lg font-semibold text-gray-800 mt-0.5">{formatCurrency(warehouse.totalStockValue)}</p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-3 sm:p-4 border border-gray-100 min-h-[44px] flex flex-col justify-center">
                      <p className="text-xs sm:text-sm text-gray-500">Revenu</p>
                      <p className="text-base sm:text-lg font-semibold text-sky-700 mt-0.5">{formatCurrency(warehouse.totalRevenue)}</p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-3 sm:p-4 border border-gray-100 min-h-[44px] flex flex-col justify-center">
                      <p className="text-xs sm:text-sm text-gray-500">Profit</p>
                      <p className="text-base sm:text-lg font-semibold text-emerald-600 mt-0.5">{formatCurrency(warehouse.totalProfit)}</p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-3 sm:p-4 border border-gray-100 min-h-[44px] flex flex-col justify-center">
                      <p className="text-xs sm:text-sm text-gray-500">Unites vendues</p>
                      <p className="text-base sm:text-lg font-semibold text-gray-800 mt-0.5">{formatNumber(warehouse.totalUnitsSold)}</p>
                    </div>
                    <div className="bg-gray-50 rounded-xl p-3 sm:p-4 border border-gray-100 min-h-[44px] flex flex-col justify-center">
                      <p className="text-xs sm:text-sm text-gray-500">Marge moy.</p>
                      <p className="text-base sm:text-lg font-semibold text-gray-800 mt-0.5">{`${Number(warehouse.averageMargin || 0).toFixed(1)} %`}</p>
                    </div>
                  </div>
                </div>
                      ...
                      {warehouse.products && warehouse.products.length > 0 ? (
                        warehouse.products.map((product) => (
                          ...
                          >
                            ...
                            <td data-title="Revenu" className="py-2 px-3 text-right text-sky-700 font-semibold">
                              {formatCurrency(product.revenue)}
                            </td>
                            <td data-title="Profit" className="py-2 px-3 text-right text-emerald-600 font-semibold">
